export interface Decoder<T> {
  ...
  type Tag<T> = { _t: T };
  type GetTag<S extends Tag<unknown>> = S["_t"];
  
  export type Type<T> = Decoder<T> & { tag?: Tag<T> };
  ...
  export const union = <CS extends [Unknown, Unknown, ...Array<Unknown>]>(members: CS): Type<TypeOf<CS[number]>> => ({
    decode: (t) => {
  ...
  export type TypeOf<C extends Unknown> = GetTag<NonNullable<C["tag"]>>;
